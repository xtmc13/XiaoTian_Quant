# 用户 Python 策略契约化运行时 v1（PYTHON_STRATEGY_API）
...
- 存储：`xt_pystrategies`（迁移 0021）+ 版本快照复用 `xt_strategy_versions`（0014）
...
| `risk.max_position_pct` | 否 | 买入前硬校验：买后仓位名义价值 / 权益 ≤ 该值，范围 `(0,1]`，`0`=不限制 |
...
| `on_order(context, order)` | 可选 | v1 预留（订单回报回调），签名必须 `on_order(context, order)` |
...
| `context.buy(qty=None, amount=None, price=None)` | 买入：`qty`（币数量）或 `amount`（计价币金额）至少一个；市价执行，`price` 忽略 |
| `context.sell(qty=None, amount=None, price=None)` | 卖出/减仓：`qty` 或 `amount` 至少一个；无持仓时空操作 |
...
- **v1 执行为现货**：`buy`=现货买入、`sell`/`close_position`=现货卖出；
  `direction=short` 时买入开仓被拒绝（记日志）。合约杠杆执行列入 v1.1。
- **TP/SL 映射**：买入成交后，按 manifest.risk 与 `set_*` 覆盖计算
  `tp=entry×(1+tpPct)`、`sl=entry×(1−slPct)`，挂到现有条件单引擎
  （`ConditionalEngine.RegisterTPSL`，trigger 后市价平仓）。未配置
  Protector 时记日志并跳过。
...
- `on_order` 回调仅做签名校验，v1 不触发（订单回报链路未接入 runner）。
- `context.buy/sell(price=...)` 限价参数忽略（v1 全市价）。

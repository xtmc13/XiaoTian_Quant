      ...
      <div className="flex-1 flex flex-col min-w-0">
        <div className="flex items-center justify-between px-4 py-2 border-b border-quant-border bg-quant-bg-secondary">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => setCode(getDefaultStrategyCode(strategyType || 'custom'))}
              className="px-2.5 py-1.5 rounded-md bg-quant-bg border border-quant-border text-[10px] hover:bg-quant-hover transition-colors flex items-center gap-1"
            >
      ...
      <div className="hidden md:flex w-80 shrink-0 border-l border-quant-border bg-quant-bg-secondary flex-col">
        ...
        <div className="p-3 space-y-3">
          ...
          <button
            ...
            disabled={generating}
            className="w-full py-2 bg-quant-gold text-white rounded-lg text-xs font-medium hover:opacity-90 disabled:opacity-50 transition-opacity"
          >
            {generating ? '生成中...' : '生成策略'}
          </button>
        </div>

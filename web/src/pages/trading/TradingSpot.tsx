...
import {
  ...
  AdvancedSettings,
  AccountInfo,
} from '@/components/trading/OrderFormInputs'
import { SpotPositionsTab, OrdersTab, HistoryTab, FillsTab, AssetsTab } from '@/components/trading/TradingBottomTabs'
import { Skeleton } from '@/components/ui/Skeleton'
import { EmptyState } from '@/components/ui/EmptyState'
import { ErrorBoundary } from '@/components/ErrorBoundary'
import { parseInterval, formatPrice, formatTime, formatDateTime, StatusTag, SPOT_WATCHLIST } from '@/lib/tradingHelpers'
...
import { Search, TrendingUp, Clock, XCircle, CheckCircle2, Activity, ChevronUp, ChevronDown, Star } from 'lucide-react'
...
export function TradingSpot() {
  ...
  const [activeBottomTab, setActiveBottomTab] = useState<'positions' | 'orders' | 'history' | 'fills' | 'assets'>(
    'positions'
  )
          ...
          <div className="h-[220px] shrink-0 border-b border-quant-border flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto">
              {filteredWatchlist.map((sym) => {
                ...
                const symPrice = isActive
                  ? snapshot?.price
                    ? parseFloat(String(snapshot.price))
                    : undefined
                  : priceMap[sym] || undefined
                return (
                  <WatchlistItem
                    key={sym}
                    sym={sym}
              ...
              <div className="flex flex-col gap-1.5">
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex gap-1">
                    <button
                      onClick={() => {
                        if (lastPrice) setPrice((lastPrice * 0.99).toFixed(precision.price))
                      }}
                    ...
                    <button
                      ...
                      className="flex-1 py-1 text-[10px] text-quant-gold hover:text-quant-gold/80 bg-quant-bg border border-quant-gold/30 rounded hover:bg-quant-gold/10 transition-colors font-medium"
                    >
                      最新价
                    </button>
                    <button
                    ...
                    <button
                      onClick={() => {
                        if (lastPrice) setPrice((lastPrice * 1.01).toFixed(precision.price))
                      }}
                      className="flex-1 py-1 text-[10px] text-muted-foreground hover:text-foreground bg-quant-bg border border-quant-border rounded hover:border-quant-gold/50 transition-colors"
                    >
                      +1%
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    if (amountMode === 'amount') {
                      ...
                      const assetFree = assetHolding
                        ? parseFloat(
                            String(
                              (assetHolding as Record<string, unknown>).free ??
                                (assetHolding as Record<string, unknown>).available ??
                                0
                            )
                          )
                        : 0
                      const calcQty =
                        side === 'BUY' ? (calcPrice > 0 ? (spotBalance * pct) / calcPrice : 0) : assetFree * pct
              ...
              <div className="flex flex-col gap-2 p-2 bg-quant-bg/50 rounded-lg border border-quant-border/50">
                <div className="flex items-center justify-between text-[10px] text-muted-foreground mb-1">
                  ...
                  <button
                    ...
                    className="text-quant-gold hover:text-quant-gold/80 transition-colors"
                  >
                    智能设置
                  </button>
                </div>
                ...
                return [0.25, 0.5, 0.75, 1].map((pct) => {
                  ...
                  const quickOrder = async () => {
                    ...
                    try {
                      ...
                      toast(
                        'success',
                        `${side === 'BUY' ? '买入' : '卖出'} ${calcQty.toFixed(precision.quantity)} ${symbol.replace('USDT', '')}`
                      )
                      queryClient.invalidateQueries({ queryKey: ['orders'] })
                      queryClient.invalidateQueries({ queryKey: ['portfolio'] })
                    } catch (e: unknown) {
                  ...
                  <table className="w-full text-[11px] whitespace-nowrap">
                    ...
                    <tbody>
                      {holdingsList.map((b: unknown, i: number) => {
                        ...
                        return (
                          <tr
                            key={String(bal.asset || bal.currency || i)}

...
export function calculateSMA(data: KLineBar[], length: number): (number | null)[] {
...
export function calculateEMA(data: KLineBar[], length: number): (number | null)[] {
  const result: (number | null)[] = []
  const k = 2 / (length + 1)
  for (let i = 0; i < data.length; i++) {
    if (i < length - 1) {
...
export function calculateBollingerBands(
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    const mid = sma[i]!
    for (let j = i - length + 1; j <= i; j++) {
      sumSq += Math.pow(data[j].close - mid, 2)
    }
...
export function calculateRSI(data: KLineBar[], length: number): (number | null)[] {
  const result: (number | null)[] = []
  let avgGain = 0
  let avgLoss = 0
...
/** Wilder ATR(period). */
export function calculateATR(data: KLineBar[], period: number): (number | null)[] {
  const tr: number[] = []
  for (let i = 0; i < data.length; i++) {
    if (i === 0) {
...
export function calculateCCI(data: KLineBar[], length: number): (number | null)[] {
...
export function calculateWilliamsR(data: KLineBar[], length: number): (number | null)[] {
...
export function calculateMFI(data: KLineBar[], length: number): (number | null)[] {
      ...
      for (let j = i - length + 1; j <= i; j++) {
        ...
        if (j > i - length + 1) {
          const prevTypicalPrice =
            (data[j - 1].high + data[j - 1].low + data[j - 1].close) / 3
...
export function calculateADX(data: KLineBar[], length: number): ADXResult {
  ...
  const smoothTR: (number | null)[] = []
  const smoothPlusDM: (number | null)[] = []
  const smoothMinusDM: (number | null)[] = []
  ...
  for (let i = 0; i < data.length; i++) {
    if (i < length - 1) {
      ...
      smoothPlusDM.push(
        smoothPlusDM[i - 1]! - (smoothPlusDM[i - 1]! / length) + plusDM[i],
      )
      smoothMinusDM.push(
        smoothMinusDM[i - 1]! - (smoothMinusDM[i - 1]! / length) + minusDM[i],
      )
    }
    ...
    if (i >= length - 1) {
      ...
      if (i === length - 1) {
        ...
        const prevDX =
          (Math.abs(plusDI[i - 1]! - minusDI[i - 1]!) /
            (plusDI[i - 1]! + minusDI[i - 1]!)) *
          100
...
export function calculateOBV(data: KLineBar[]): number[] {
  const obv: number[] = []
  let obvValue = 0
  for (let i = 0; i < data.length; i++) {
    if (i === 0) {
...
export function calculateAD(data: KLineBar[]): number[] {
  const ad: number[] = []
  let adValue = 0
  for (let i = 0; i < data.length; i++) {
...
export function calculateADOSC(
  data: KLineBar[],
  fast: number,
  slow: number,
): (number | null)[] {
  const ad = calculateAD(data)
  const adAsBars: KLineBar[] = ad.map((v) => ({ close: v } as KLineBar))
